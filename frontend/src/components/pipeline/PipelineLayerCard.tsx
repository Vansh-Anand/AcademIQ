...
import { Loader2, ShieldCheck, ShieldAlert, AlertTriangle, HelpCircle, Lock, CircleSlash } from 'lucide-react';
...
export const PipelineLayerCard: React.FC<PipelineLayerCardProps> = ({
  ...
  if (isSelected) {
    borderColor = 'border-purple-500 ring-2 ring-purple-500 ring-offset-2';
  }

  const renderIcon = () => {
    if (isPending) return <div className="w-5 h-5 rounded-full border-2 border-gray-200" />;
    if (isSkipped) return <CircleSlash className="w-5 h-5 text-gray-300" />;
    if (isProcessing) return <Loader2 className="w-5 h-5 text-blue-500 animate-spin" />;
    if (isBlocked) return <ShieldAlert className="w-5 h-5 text-red-500 animate-pulse" />;
  ...
  return (
    <div 
      className={`relative p-4 rounded-lg border transition-all duration-300 ${borderColor} ${bgColor} ${isPending ? 'opacity-50' : 'opacity-100'} ${onClick ? 'cursor-pointer hover:shadow-md' : ''}`}
      onClick={onClick}
    >
